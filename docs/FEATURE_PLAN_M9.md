...
- **M9.2** Фикс задержки: `--sleep=3` → `--sleep=1` в queue worker ✅
- **M9.3** Отправка по Enter: `onKeyDown` в форму ввода, Shift+Enter = перенос ✅
- **M9.4** Порядок сообщений: новые снизу + автоскролл + бейдж + `white-space: pre-wrap` ✅
- QA + Reviewer + PR в develop ✅ (PR #42)
